...
const SpecialMenuModal = ({ onClose, subcategories, onAddToCart, priceId }) => {
  ...
  const handleSelect = (subcategoryName, item) => {
    setSelectedItems((prev) => {
      ...
      if (itemExists) {
        ...
        if (
          ...
          (subcategories.find((sub) => sub.name === subcategoryName)
            ?.subquantity || 1)
        ) {
  ...
  const handleSubmit = () => {
    const totalPrice = priceId; // Only main price considered
    ...
    dispatch({
      ...
      payload: { keys: platterWithCategory.key },
    });
    onClose();
  };
          ...
          {subcategories?.map((subcategory) => (
            <div key={subcategory.name} className="border-b pb-4">
              ...
              <h3 className="text-xs mb-2">
                You can only select {subcategory.subquantity || 1} dishes under{" "}
                {subcategory.name}
              </h3>
              <ul>
